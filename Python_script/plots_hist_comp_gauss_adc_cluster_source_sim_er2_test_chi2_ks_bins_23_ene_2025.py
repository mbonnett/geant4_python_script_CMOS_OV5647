                ...
                for alpha in [1.75]:
                    ...
                    for zff in [1.0]:
                         ...
                         for z in level_z:
                            ...
                            dirsavepatn_plt = '/home/mbonnett/mik/dat_sim_2025/' +gauss_dist+'_comp2'+dens+'_dghist_'+strbin +str(z*2).zfill(2)+'mm'+ cut_clst_size+cut_max+str_cuts+'/'
                            ...
                            name_filter_sim = sig_bg_thresh_sim +condit_edep+difu_gauss+ cut_adc + cut_max_clst_porc_sim
                            #####################################################################################################################################################
                            ...
                            #####################################################################################################################################################
                            data_cnt_sim1 = np.load(dir_sim1+'/'+source1+'_ev'+ev1+ name_filter_sim +'_z'+str(z).zfill(3)+'.npz' )
                            data_cnt_sim2 = np.load(dir_sim2+'/'+source2+'_ev'+ev2+ name_filter_sim +'_z'+str(z).zfill(3)+'.npz' )
                            ################################################################################################################################################
                            ...
                            #######################################################################################################
                            #######################################################################################################
                            data_cluste_frame_sim1 = file_cluster_size_mean_sim1+'dat_sim_'+source1+'_f'+str(nframes)+'_clstr_frm_z'+str(z).zfill(3)+\
                                                    '_w'+evt1 + difu_gauss + cut_adc + sig_bg_thresh_sim + sigthresh +cut_clst_size+cut_max + '.csv'

                            data_cluste_frame_sim2 = file_cluster_size_mean_sim2+'dat_sim_'+source2+'_f'+str(nframes)+'_clstr_frm_z'+str(z).zfill(3)+\
                                ...
                                for plt_one in[ True,
                                    ...
                                    for type_plt in ['cl_size', 'cl_mean', 'cl_etotal', 'cl_max_adc' ]:
                                        ...
                                        if(type_plt=='cl_size'):
                                            ...
                                            xlabs = 'Cluster Size'
                                            #r= np.max(np.array((np.max(data_cut_obs1[:,7]), np.max(data_cut_sim2[:,7]) )))
                                            #maxims = r+1
                                            ...
                                            if(np.max(data_cut_obs1[:,7]) == max_c ):data_cut = data_cut_obs1
                                            if(np.max(data_cut_obs2[:,7]) == max_c ):data_cut = data_cut_obs2
                                            if(np.max(data_cut_sim1[:,7]) == max_c ):data_cut = data_cut_sim1
                                            if(np.max(data_cut_sim2[:,7]) == max_c ):data_cut = data_cut_sim2
                                        ...
                                        if(type_plt=='cl_mean'):
                                            ...
                                            if( min_adc==adc_cut):data_cut = data_cut_obs1
                                                ...
                                                if(np.max(data_cut_obs1[:,8]) == max_me ):data_cut = data_cut_obs1
                                                if(np.max(data_cut_obs2[:,8]) == max_me ):data_cut = data_cut_obs2
                                                if(np.max(data_cut_sim1[:,8]) == max_me ):data_cut = data_cut_sim1
                                                if(np.max(data_cut_sim2[:,8]) == max_me ):data_cut = data_cut_sim2
                                        ...
                                        if(type_plt=='cl_max_adc'):
                                            ...
                                            if( min_adc==adc_cut):data_cut = data_cut_obs1
                                                ...
                                                if(np.max(data_cut_obs1[:,10]) == max_s ):data_cut = data_cut_obs1
                                                if(np.max(data_cut_obs2[:,10]) == max_s ):data_cut = data_cut_obs2
                                                if(np.max(data_cut_sim1[:,10]) == max_s ):data_cut = data_cut_sim1
                                                if(np.max(data_cut_sim2[:,10]) == max_s ):data_cut = data_cut_sim2
                                        ...
                                        if(type_plt=='cl_etotal'):
                                            ...
                                            if( min_adc==adc_cut):data_cut = data_cut_obs1
                                                ...
                                                if(np.max(data_cut_obs1[:,9]) == max_e ):data_cut = data_cut_obs1
                                                if(np.max(data_cut_obs2[:,9]) == max_e ):data_cut = data_cut_obs2
                                                if(np.max(data_cut_sim1[:,9]) == max_e ):data_cut = data_cut_sim1
                                                if(np.max(data_cut_sim2[:,9]) == max_e ):data_cut = data_cut_sim2
                            ...
                            if(plots_2d==True):
                                ...
                                lbl_obs0 = source1 + ' data '
                                lbl_obs1 = source2 + ' data '
                                lbl_sim1 = source1 + ' simulation '
                                lbl_sim2 = source2 + ' simulation '
                                    ...
                                    for plt_one in[ True,
                                        ...
                                        for q in [1,2]:
                                            ...
                                            for p in range(row):
                                                ...
                                                if(p==0 and q==1):
                                                    x = data_cut_obs1[:,10]; y = data_cut_obs1[:,7];
                                                    xbin_all = seedlinbins_all; ybin_all = cltslinbins_all;
                                                    wxy_sc1, x_sc1, y_sc1 = np.histogram2d(x, y, bins=(xbin_all, ybin_all) )

                                                    xbin = seedlinbins; ybin = cltslinbins;
                                                    wxy_sc_bh1, x_sc_bh1, y_sc_bh1 = np.histogram2d(x, y, bins=(xbin, ybin) )

                                                    hist2d, bins_x, bins_y = wxy_sc_bh1, x_sc_bh1, y_sc_bh1

                                                    labl = lbl_obs1;
                                                    label_x=lab2d[1]; label_y=lab2d[0]

                                                if(p==0 and q==2):
                                                    x = data_cut_obs2[:,10]; y = data_cut_obs2[:,7];
                                                    xbin_all = seedlinbins_all; ybin_all = cltslinbins_all;
                                                    wxy_sc2, x_sc2, y_sc2 = np.histogram2d(x, y,  bins=(xbin_all, ybin_all) )

                                                    xbin = seedlinbins; ybin = cltslinbins;
                                                    wxy_sc_bh2, x_sc_bh2, y_sc_bh2 = np.histogram2d(x, y, bins=(xbin, ybin) )

                                                    hist2d, bins_x, bins_y = wxy_sc_bh2, x_sc_bh2, y_sc_bh2
                                                ...
                                                if(plt_sim==True):
                                                    ...
                                                    if(p==1 and q==1):
                                                        x = data_cut_sim1[:,9]; y = data_cut_sim1[:,7];
                                                        xbin = seedlinbins_all; ybin = cltslinbins_all;
                                                        xy_sc_sim1, x_sc_sim1, y_sc_sim1 = np.histogram2d(x, y,  bins=(xbin_all, ybin_all) )

                                                        xbin = seedlinbins; ybin = cltslinbins;
                                                        wxy_sc_sim_bh1, x_sc_sim_bh1, y_sc_sim_bh1 = np.histogram2d(x,y, bins=(xbin, ybin) )

                                                        hist2d, bins_x, bins_y = wxy_sc_sim_bh1, x_sc_sim_bh1, y_sc_sim_bh1

                                                        labl = lbl_sim1;
                                                        label_x=lab2d[1]; label_y=lab2d[0]

                                                    if(p==1 and q==2):
                                                        x = data_cut_sim2[:,9]; y = data_cut_sim2[:,7];
                                                        xbin = seedlinbins_all; ybin = cltslinbins_all;
                                                        xy_sc_sim2, x_sc_sim2, y_sc_si21 = np.histogram2d(x, y,  bins=(xbin_all, ybin_all) )

                                                        xbin = seedlinbins; ybin = cltslinbins;
                                                        wxy_sc_sim_bh2, x_sc_sim_bh2, y_sc_sim_bh2 = np.histogram2d(x,y, bins=(xbin, ybin) )

                                                        hist2d, bins_x, bins_y = wxy_sc_sim_bh2, x_sc_sim_bh2, y_sc_sim_bh2
                                                ...
                                                if(zom==False):
                                                    if(q==1):x_zoom =  data_cut_obs1[:,10].max(); y_zoom = data_cut_obs1[:,7].max()
                                                    if(q==2):x_zoom =  data_cut_obs2[:,10].max(); y_zoom = data_cut_obs2[:,7].max()

                                                if(zom==True):
                                                    if(q==1):x_zoom = 1000;   y_zoom = 80
                                            ...
                                            if(plt_one == True):
                                                if(q==1  or q ==2):str_hist2 = 'Cluster_vs_Max_ADC'
