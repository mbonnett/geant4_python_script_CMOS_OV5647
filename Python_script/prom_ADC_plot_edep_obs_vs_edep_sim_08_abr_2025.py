...
def comp_obs_sim0(obs, sim):
    dif = (obs - sim)
    err = np.sqrt(obs + 0.04*sim*sim )
    comp = np.divide(dif, err , where=(err != 0))
    err_comp = np.abs(comp)*np.sqrt( (err*err) / (dif*dif)
            + (obs + 0.000256*sim*sim*sim*sim) / (4*err*err*err*err) )
    return comp, err_comp
            ...
            for zff in [1.0]:
                ...
                for densi in [ False, #True
                    ...
                    for z in level_z:
                        ...
                        comp_gauss, err_comp_gauss = comp_obs_sim0(adc_obs_w, adc_sim_gauss_w)
                        # Filtrar los valores menores o iguales a 5
                        data_filtrado = comp_gauss[comp_gauss <= np.abs(comp_gauss.min())+1]
                        ax.hist(data_filtrado, bins=30,  histtype='step'
                                    , density=densi, log=scal_log, color='C0', linewidth=l_width,
                                    label = lbl_sim0 + cut_max_clst_porc_sim)
                        ax.set_xlabel(r'$\frac{ (data -sim)}{\sigma}$', fontsize=font_siz)
                        if scal_log == True: ax.set_yscale('log')
                        ax.set_ylabel('counts', fontsize=font_siz)
                        ax.set_title(source, fontsize=font_siz+4)
                        plt.tight_layout()
                        plt.show()
                        ...
                        # Crear una matriz 2D para el histograma
                        hist_2d = np.outer(adc_sim_gauss_w, adc_obs_w)

                        # Crear el gráfico de colores
                        im = ax.pcolormesh(bins_x, bins_y, hist_2d.T, norm=LogNorm(), cmap=plt.cm.jet)

                        # Ajustar la posición del gráfico para dejar espacio para la barra de colores
                        fig.subplots_adjust(right=0.82)

                        # Crear la barra de colores en una posición específica para evitar superposición
                        cbar_ax = fig.add_axes([0.85, 0.15, 0.03, 0.7])  # [left, bottom, width, height]
                        cbar = fig.colorbar(im, cax=cbar_ax, label='Counts')
                        cbar.ax.tick_params(labelsize=font_siz - 4)
                        cbar.ax.set_ylabel('Counts', fontsize=font_siz - 2)
                        ...
                        # Configurar los ejes principales (ADC) a la derecha y arriba

                        #ax.xaxis.tick_top()  # Mover las marcas del eje X a la parte superior
                        #ax.yaxis.tick_right()  # Mover las marcas del eje Y a la derecha
                        #ax.xaxis.set_label_position('top')  # Mover la etiqueta del eje X a la parte superior
                        #ax.yaxis.set_label_position('right')  # Mover la etiqueta del eje Y a la derecha
                        ...
                        # Crear el eje X secundario (ADC) en la parte inferior
                        ax1 = ax.twiny()
                        ax1.xaxis.tick_top()  # Asegurar que las marcas estén en la parte inferior
                        ax1.xaxis.set_label_position('top')  # Asegurar que la etiqueta esté en la parte inferior
                        ax1.set_xlim(ax.get_xlim())
                        ax1.set_xlabel('ADC Obs', fontsize=font_siz-2)

                        # Crear el eje X secundario (Energy) en la parte inferior
                        ax2 = ax.twiny()
                        ax2.xaxis.tick_bottom()  # Asegurar que las marcas estén en la parte inferior
                        ax2.xaxis.set_label_position('bottom')  # Asegurar que la etiqueta esté en la parte inferior
                        ax2.set_xlim(ax.get_xlim())
                        ax2.set_xlabel('Obs $E_{dep} $(keV)', fontsize=font_siz-2)

                        # Crear el eje Y secundario (ADC) en la parte inferior
                        ax3 = ax.twinx()
                        ax3.yaxis.tick_right()  # Asegurar que las marcas estén en la parte inferior
                        ax3.yaxis.set_label_position('right')  # Asegurar que la etiqueta esté en la parte inferior
                        ax3.set_ylim(ax.get_ylim())
                        ax3.set_ylabel('ADC sim', fontsize=font_siz-2)

                        # Crear el eje Y secundario (Energy) en el lado izquierdo
                        ax4 = ax.twinx()
                        ax4.yaxis.tick_left()  # Asegurar que las marcas estén en el lado izquierdo
                        ax4.yaxis.set_label_position('left')  # Asegurar que la etiqueta esté en el lado izquierdo
                        ax4.set_ylim(ax.get_ylim())
                        ax4.set_ylabel('Sim $E_{dep} $(keV)', fontsize=font_siz-2)

                        # Función para convertir ADC a keV
                        def adc_to_kev(x):
                            return (x * (emax-emin)/1023.0 + emin) * pair_eh/U_eV

                        # Configurar las etiquetas de los ejes secundarios
                        ax2.set_xticklabels([f"{adc_to_kev(tick):.1f}" for tick in ax.get_xticks()])
                        ax4.set_yticklabels([f"{adc_to_kev(tick):.1f}" for tick in ax.get_yticks()])

                        plt.tight_layout(rect=[0, 0, 0.82, 1])  # Ajustar el diseño pero dejar espacio para la barra de


                        plt.savefig(f'{dirsave_plt_mean}/hist2d_{source}_Edepsim_vs_Edepobs.png', dpi=150)
                        plt.savefig(f'{dirsave_plt_mean}/hist2d_{source}_Edepsim_vs_Edepobs.pdf', dpi=150)
